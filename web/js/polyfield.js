    ...
    class Polyfield {
      ...
      generateDropdown(id, modelName, attribute, counter, label, values, selected, filterAttr, sortAttr, attributePrefix, valueAttribute, exists, template) {
        ...
        if (filterAttr) {
          ...
          filterSelect.appendChild(this.generateOptions(filterValues, attribute, null, sortAttr, attributePrefix, valueAttribute, null, template));
          ...
          jQuery(filterSelect).on('change', () => {
            ...
            return select.appendChild(this.generateOptions(filteredValues, attribute, selected, sortAttr, attributePrefix, valueAttribute, null, template));
          });
        }
      ...
      bindOrderListener(classSelector) {
        if (typeof this.ordersListener[classSelector] === 'undefined') {
          $('body').on('focus click', `.${classSelector}`, e => {
            return this.ordersListener[classSelector] = e.target.value;
          });
          $('body').on('change', `.${classSelector}`, e => {
            var freeValue, self;
            self = $(e.target);
            freeValue = this.ordersListener[classSelector];
      ...
      bindClose(id) {
        return jQuery('body').on('click', '#exit_' + id, () => {
          if (confirm(this.translate('deleteConfirmation'))) {
